...
while IFS= read -r file; do
    while IFS= read -r sheet; do
        in2csv --sheet "$sheet" "$file" > "${file%.*}_${sheet// /_}.csv"
    done < <(in2csv -n "$file")
done < <(find . -name '*.xls' -o -name '*.xlsx')

## ${sheet// /_} part will replace a space with an underscore in the sheetname.csv
## character to be replaced enclosed between first two // and one /
## one / precedes replacement character _ underscore
